    ...
    ) : IQueryHandler<GetAllRBooksWithPagingQuery, ServiceResult<PagedResult<ReadingBookWithTotalCount>>>
{
        ...
        CancellationToken cancellationToken)
    {
        logger.LogInformation(
            "GetAllRBooksWithPagingQueryHandler -> FETCHING READING BOOKS FOR USER: {UserId}, PAGE: {Page}, PAGE SIZE: {PageSize}", 
            request.UserId, request.Page, request.PageSize);

        var (items, totalCount) = await readingBookRepository.GetAllRBooksWithPagingAsync(
            request.UserId, request.Page, request.PageSize);

        logger.LogInformation(
            "GetAllRBooksWithPagingQueryHandler -> FETCHED {Count} READING BOOKS OUT OF {Total} FOR USER: {UserId}", 
        ...
        var result = PagedResult<ReadingBookWithTotalCount>.Create([mappedResult], pagedRequest, totalCount);
