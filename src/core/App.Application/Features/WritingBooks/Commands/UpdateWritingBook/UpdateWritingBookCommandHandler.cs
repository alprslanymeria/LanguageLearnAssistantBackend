...
public class UpdateWritingBookCommandHandler(
    ...
    ) : ICommandHandler<UpdateWritingBookCommand, ServiceResult<WritingBookDto>>
{
    public async Task<ServiceResult<WritingBookDto>> Handle(
        UpdateWritingBookCommand request, 
        CancellationToken cancellationToken)
    {
        logger.LogInformation("UpdateWritingBookCommandHandler -> UPDATING WRITING BOOK WITH ID: {Id}", request.Id);
        ...
        {
            logger.LogWarning("UpdateWritingBookCommandHandler -> WRITING BOOK NOT FOUND WITH ID: {Id}", request.Id);
    ...
    private async Task<string> UploadFileToStorageAsync(IFileUpload file, string userId, string folderName)
    {
        ...
        logger.LogInformation("UpdateWritingBookCommandHandler -> SUCCESSFULLY UPLOADED FILE TO STORAGE: {FileUrl}", fileUrl);
    ...
    private async Task DeleteFileFromStorageAsync(int writingBookId, string fileUrl)
    {
        ...
        try
        {
            ...
            logger.LogInformation("UpdateWritingBookCommandHandler -> SUCCESSFULLY DELETED FILE FROM STORAGE: {FileUrl}", fileUrl);
        }
        catch (Exception ex)
        {
            logger.LogWarning(ex, "UpdateWritingBookCommandHandler -> FAILED TO DELETE FILE FROM STORAGE FOR WRITING BOOK {Id}: {FileUrl}", writingBookId, fileUrl);
        }
    }
    ...
    private async Task<string> ExtractLeftSideColorAsync(IFileUpload imageFile)
    {
        try
        {
            ...
            logger.LogInformation("UpdateWritingBookCommandHandler -> EXTRACTED COLOR: {Color}", hexColor);
            
            return hexColor;
        }
        catch (Exception ex)
        {
            logger.LogWarning(ex, "UpdateWritingBookCommandHandler -> FAILED TO EXTRACT COLOR, USING DEFAULT");
    ...
    private async Task<ServiceResult<Writing>> VerifyOrCreateWritingAsync(int writingId, string userId, int languageId)
    {
        ...
        logger.LogWarning("UpdateWritingBookCommandHandler -> WRITING NOT FOUND WITH ID: {WritingId}", writingId);

        var language = await languageRepository.GetByIdAsync(languageId);

        if (language is null)
        {
            logger.LogWarning("UpdateWritingBookCommandHandler -> LANGUAGE NOT FOUND FOR ID: {LanguageId}", languageId);
        ...
        if (practice is null)
        {
            logger.LogWarning("UpdateWritingBookCommandHandler -> PRACTICE NOT FOUND FOR LANGUAGE ID: {LanguageId}", languageId);
        ...
        logger.LogInformation("UpdateWritingBookCommandHandler -> NEW WRITING CREATED WITH ID: {WritingId}", writing.Id);
